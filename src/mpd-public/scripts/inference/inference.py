...
# Compatibility shim for legacy deps (e.g., networkx==2.2 with numpy>=1.24).
...
TRAINED_MODELS_DIR = os.environ.get('MPD_TRAINED_MODELS_DIR', '../../data_trained_models/')
RESULTS_ROOT_DIR = os.environ.get('MPD_RESULTS_DIR', None)
...
def experiment(
    ...
    **kwargs
    ...
    if RESULTS_ROOT_DIR:
        results_dir = os.path.join(RESULTS_ROOT_DIR, model_id, str(seed))
    else:
        ...
        if isinstance(robot, RobotPanda):
            ...
            try:
                from torch_robotics.isaac_gym_envs.motion_planning_envs import (
                    PandaMotionPlanningIsaacGymEnv,
                    MotionPlanningController,
                )
            except Exception as exc:
                raise RuntimeError(
                    "IsaacGym visualization requires a working isaacgym install and CUDA runtime."
                ) from exc
